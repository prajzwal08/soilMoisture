...
#SBATCH --output=logs/wide_dem_%j.out
...
#
# Resume-safe: a region with dem_glo30_30m.tif already present is skipped, so a
...
conda run -n soilmoisture --no-capture-output python download_wide_dem.py --workers 8

date
echo "regions written: $(ls -d /gpfs/work3/0/prjs1968/data/terrain/region_*/dem_glo30_30m.tif 2>/dev/null | wc -l) / 353"
