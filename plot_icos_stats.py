"""
Exploratory statistics and plots for preprocessed ICOS data.
Saves SVGs to plots/icos_stats/.
"""
...
LEVEL1  = Path("/home/khanalp/data/soilmoisture/icos_level1")
OUT_DIR = Path("/home/khanalp/code/PhD/soilMoisture/plots/icos_stats")
...
for f in files:
    ...
    records.append({
        ...
        "station_theme": a.get("station_theme", "Unknown"),
        "icos_class":    a.get("icos_class", "Unknown"),
        "start":         time[0],
        "end":           time[-1],
        "n_days":        n_days,
        "n_years":       round(n_days / 365.25, 2),
...
# ---------------------------------------------------------------------------
print("=" * 60)
print("ICOS Preprocessed Dataset Summary")
print("=" * 60)
...
print()
print("Station theme distribution:")
print(df["station_theme"].value_counts().to_string())

# ---------------------------------------------------------------------------
# Fig 1: Histogram of station duration
# ---------------------------------------------------------------------------
fig, ax = plt.subplots(figsize=(5, 3.5))
bins = np.arange(1, df["n_years"].max() + 1, 0.5)
...
fig, ax  = plt.subplots(figsize=(10, max(4, len(df_gantt) * 0.22)))
...
ax.set_yticklabels(df_gantt["site_id"], fontsize=5)
ax.set_xlabel("Year")
...
counts_total = [df[(df["start"].dt.year <= y) & (df["end"].dt.year >= y)].shape[0] for y in years]
counts_swc   = [swc[(swc["start"].dt.year <= y) & (swc["end"].dt.year >= y)].shape[0] for y in years]
fig, ax = plt.subplots(figsize=(5, 3.5))
ax.bar(years, counts_total, color="steelblue",  edgecolor="white", lw=0.4, label="Total")
ax.bar(years, counts_swc,   color="#2166ac",    edgecolor="white", lw=0.4, label="With SWC")
ax.set_xlabel("Year")
...
# ---------------------------------------------------------------------------
# Fig 5: Geographic distribution (Europe-focused)
# ---------------------------------------------------------------------------
fig, ax = plt.subplots(figsize=(7, 5))
sc = ax.scatter(df["lon"], df["lat"], c=df["n_years"],
                cmap="YlOrRd", s=25, edgecolors="k", lw=0.3, vmin=1, zorder=3)
plt.colorbar(sc, ax=ax, label="Valid years")
swc_pts = df[df["has_swc"]]
ax.scatter(swc_pts["lon"], swc_pts["lat"], s=55, facecolors="none",
           edgecolors="#2166ac", lw=1, label="Has SWC", zorder=4)
ax.set_xlabel("Longitude")
ax.set_ylabel("Latitude")
ax.set_title(f"ICOS stations (n={len(df)})")
ax.legend(fontsize=7)
ax.spines[["top", "right"]].set_visible(False)
fig.tight_layout()
...
# Save summary CSV
# ---------------------------------------------------------------------------
df.to_csv("/home/khanalp/code/PhD/soilMoisture/csvs/icos_summary.csv", index=False)
print(f"\nSaved 5 plots -> {OUT_DIR}")
print(f"Saved summary CSV -> csvs/icos_summary.csv")
